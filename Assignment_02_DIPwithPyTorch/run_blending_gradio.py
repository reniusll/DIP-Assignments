...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_canvas = torch.zeros_like(blended_img)
    foreground_canvas_mask = torch.zeros_like(background_mask)

    fg_coords = torch.nonzero(foreground_mask[0, 0] > 0.5, as_tuple=False)
    bg_coords = torch.nonzero(background_mask[0, 0] > 0.5, as_tuple=False)
    if fg_coords.numel() == 0 or bg_coords.numel() == 0:
        return loss

    fg_top_left = fg_coords.min(dim=0).values
    bg_top_left = bg_coords.min(dim=0).values
    offset_y = int(bg_top_left[0].item() - fg_top_left[0].item())
    offset_x = int(bg_top_left[1].item() - fg_top_left[1].item())

    fg_h, fg_w = foreground_img.shape[-2:]
    bg_h, bg_w = blended_img.shape[-2:]

    dst_y0 = max(offset_y, 0)
    dst_x0 = max(offset_x, 0)
    dst_y1 = min(offset_y + fg_h, bg_h)
    dst_x1 = min(offset_x + fg_w, bg_w)
    ...
    foreground_canvas[:, :, dst_y0:dst_y1, dst_x0:dst_x1] = foreground_img[:, :, src_y0:src_y1, src_x0:src_x1]
    foreground_canvas_mask[:, :, dst_y0:dst_y1, dst_x0:dst_x1] = foreground_mask[:, :, src_y0:src_y1, src_x0:src_x1]

    foreground_laplacian = F.conv2d(foreground_canvas, laplacian_kernel, padding=1, groups=foreground_img.shape[1])
    blended_laplacian = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=blended_img.shape[1])

    valid_mask = (foreground_canvas_mask * background_mask).expand(-1, foreground_img.shape[1], -1, -1)
    if valid_mask.sum() == 0:
        return loss

    loss = F.l1_loss(blended_laplacian[valid_mask.bool()], foreground_laplacian[valid_mask.bool()])
